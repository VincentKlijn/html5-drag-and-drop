...
var dndSupported;
// array which holds all draggable elements
var dndElements = new Array();
// the current element that is being dragged
var draggingElement;
// array declaring what beats what in RPS
var winners = {
...
// detect whether the browser supports DnD
function detectDragAndDrop() {
	// check if the property/attribute draggale is available on an element we create
	if ('draggable' in document.createElement('span')) {
		// if so, then return true (return stops execution of the function)
		// should work in Chrome, Firefox, Opera, Internet Explorer, Vivaldi
		return true;
	}
...
// 'this' always is the element itself that is triggering the eventListener
// in our case these are the three outer divs rps1, rps2, rps3
function handleDragStart(e) {
	// move the outer div to an object variable
...
function getRPSFooter(e) {
	...
	for( var i = 0; i < children.length; i++ ) {
		...
		if( children[i].nodeName.toLowerCase() == 'footer' ) {
			// return that element
			return children[i];
		}
	}
